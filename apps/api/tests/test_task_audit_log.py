...
@pytest.mark.asyncio
async def test_audit_log_on_task_approve(client: AsyncClient, db_session: AsyncSession):
    """管理员审核通过任务时记录审计日志"""
    admin = await create_admin(db_session)
    pm = await create_pm(db_session)

    task = Task(name="待审核任务", status=TaskStatus.UNCONFIRMED, pm_id=pm.id, task_type=TaskType.NORMAL)
    db_session.add(task)
    ...
    log_response = await client.get(
        f"/v1/audit-logs/?action=task.approve",
        headers={"Authorization": f"Bearer {token}"},
    )
    assert log_response.status_code == 200
    logs = log_response.json()
...
async def test_audit_log_on_task_publish(client: AsyncClient, db_session: AsyncSession):
    ...
    task = Task(name="待发布任务", status=TaskStatus.CONFIRMED_UNPUBLISHED, pm_id=pm.id, task_type=TaskType.NORMAL)
    db_session.add(task)
    await db_session.commit()
    await db_session.refresh(task)
