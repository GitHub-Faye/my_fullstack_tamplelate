...
async def read_roles(
    ...
    """
    roles, count = await service.list_roles(
        session=session,
    ...
    return RolesPublic(
        data=await roles_public(session=session, roles=roles),
        **paginated_fields(
            count=count,
...
async def create_role(
    *,
    ...
    - 400：包含未定义的 scope
    """
    role = await service.create_role(session=session, role_in=role_in)
...
async def delete_role(
    ...
    - 400：删除预置角色
    """
    await service.delete_role(session=session, role_id=role_id)
