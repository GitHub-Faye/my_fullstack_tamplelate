...
- [ ] 后端：创建 domains/bid 模块
- [ ] 后端：实现 Bid API（POST /tasks/{id}/bids, PUT /tasks/{id}/bids/{bid_id}, GET /tasks/{id}/bids）
- [ ] 后端：报价金额自动计算（amount = H0 × T_reported）
- [ ] 后端：权限控制（仅工程师角色可报价，报价窗口内可修改）
